	...
	return {
		...
		init : {
			...
			clock : function() {
				...
				var clock_time = weekday + ' ' + suffix + ' ' + hour + ':' + minute;
				var clock_date = year + '年' + month + day + '日';
			...
			desktop : function() {
				...
				d.on('dblclick', 'a.icon', function() {
					...
					// ajax load the page
					// create by personal@zhoujiajun.com
					var window_content_page = $(this).attr('openContentPage');
					var window_aside_page = $(this).attr('openAsidePage');
					var window_content_id = $(this).attr('contentPageID');
					var window_aside_id = $(this).attr('asidePageID');
					$("#" + window_content_id).load(window_content_page);
					$("#" + window_aside_id).load(window_aside_page);
				});
				...
				d.on('dblclick', 'div.window_top img', function() {
					// Traverse to the close button, and hide its taskbar
					// button.
			...
			wallpaper : function() {
				...
				if ($('#desktop').length) {
					$('body').prepend('<img id="wallpaper" class="abs" src="assets/images/misc/bg1.jpg" />');
				}
			}
		},
		util : {
